    ...
    metrics: [
      {
        ...
        label: 'Investorer uten avvik',
        value: String(compliantInvestors),
        context: `${compliantInvestors} av ${pageData.overview.totalInvestors} er fullstendige`,
        tone: 'ok',
      },
      {
        ...
        context: missingRequiredDataIds.size > 0 ? 'Påkrevd for AML-kontroller' : 'Ingen mangler',
      ...
      },
      {
        ...
        context: pageData.amlPep.overdueReviews > 0 ? 'Krever oppfølging nå' : 'Ingen forfalte',
        tone: pageData.amlPep.overdueReviews > 0 ? 'critical' : 'ok',
      },
      {
        id: 'pep-due-soon',
        label: 'PEP forfaller snart',
        value: String(pageData.amlPep.dueSoonReviews),
        context: pageData.amlPep.dueSoonReviews > 0 ? 'Innen 14 dager' : 'Ingen nær forestående',
        tone: pageData.amlPep.dueSoonReviews > 0 ? 'warning' : 'ok',
      },
      {
        id: 'reporting-ready',
        label: 'Klare for innsending',
        value: String(pageData.reportingWorkspace.readyForSubmission),
        context: pageData.reportingWorkspace.readyForSubmission > 0 ? 'Kan godkjennes nå' : 'Ingen klare ennå',
